...
- **Remote image hosting management (Issue #17)**: G0–G4 and S3-1 through S3-3 are complete, including Cloudflare R2 and MinIO acceptance for explicit public `urlPrefix` and 300-second presigned GET preview. Issue #26 owns the remaining safe deletion and compatibility work on the long-lived `feat/issue-26-s3-remote-management` branch; do not create separate internal S3 stage branches or issues. Remote reference management intentionally scans Markdown only. The next implementation step is provider-independent deletion gates based on a fresh Markdown index, followed by S3 DeleteObject and compatibility closure. The phased plan is maintained in [docs/design/issue-17-remote-image-management.md](../../../docs/design/issue-17-remote-image-management.md)
