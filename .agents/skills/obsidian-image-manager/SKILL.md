...
- **Remote image hosting management (Issue #17)**: G0–G3 and S3-1/S3-2 are complete. Issue #26 owns all remaining S3 preview, safe deletion, and compatibility work on the long-lived `feat/issue-26-s3-remote-management` branch; do not create separate internal S3 stage branches or issues. The next implementation step is manual on-demand preview (G4/S3-3), followed by `.canvas` deletion gates, S3 DeleteObject, and R2/MinIO compatibility closure. The phased plan is maintained in [docs/design/issue-17-remote-image-management.md](../../../docs/design/issue-17-remote-image-management.md)
