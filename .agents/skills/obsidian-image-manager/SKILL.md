...
- `customReferenceTemplate`: optional upload-only custom reference template; `{url}` is required, `{alt}` is supported, and empty or invalid templates safely fall back to Markdown
