...
- **Remote image hosting management (Issue #17)**: G0–G3 and S3-1/S3-2 are complete. Issue #23 implements and validates the first real S3-compatible list Provider for Cloudflare R2 and MinIO; the accepted post-merge flow uses prefix-scoped automatic metadata batches and local search-result pagination. The next shared stage is on-demand preview (G4/S3-3). The phased plan is maintained in [docs/design/issue-17-remote-image-management.md](../../../docs/design/issue-17-remote-image-management.md)
