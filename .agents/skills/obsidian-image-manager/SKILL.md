...
- **Remote image hosting management (Issue #17)**: G0–G4 and S3-1 through S3-3 are complete. Issue #26 now uses an S3-first responsive card grid: list scans remain explicit, visible thumbnails load through a 4-concurrency queue, results append locally in 60-card batches without page controls, and search/sort/reference filters apply to the complete scanned set. Deletion follows remote-management `enabled`; eligible cards expose selection while ineligible cards show their reason. The G5/S3-4 safety layer still includes fresh Markdown gates, exact-count confirmation, 20-item/2-concurrency scheduling, a 200-entry redacted `data.json` audit, and conservative S3 DeleteObject semantics. A virtual-folder prefix picker has been evaluated but is not implemented yet: it must use a separate `ListObjectsV2(prefix, delimiter='/')` flow and parse `CommonPrefixes`, while retaining manual prefix input. Cloudflare R2 and MinIO combined thumbnail/delete acceptance is still required. Issue #26 and `feat/issue-26-s3-remote-management` continue through compatibility closure without extra internal branches/issues. The phased plan is maintained in [docs/design/issue-17-remote-image-management.md](../../../docs/design/issue-17-remote-image-management.md)
