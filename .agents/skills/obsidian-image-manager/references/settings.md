...
`HostingConfigModal` 使用固定基础信息和三个任务页签：连接配置承载服务商 endpoint/region/bucket/凭据，上传与访问承载路径、公共 URL 或 Custom 请求映射，远程管理承载浏览与预览设置。只有页签正文滚动，保存/取消保持可见；桌面两列、移动单列。页签切换直接复用内存中的配置副本，不新增持久化字段。
