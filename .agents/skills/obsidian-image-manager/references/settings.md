...
`HostingConfigModal` 使用固定基础信息和“图床配置 / 远程管理”两个页签，选中页签使用 Obsidian 主题强调色。非 Custom 图床的上传路径与公共访问 URL 固定显示在页签上方，图床配置页只承载 endpoint/region/bucket/凭据等服务商连接字段；Custom 的请求映射仍留在图床配置页。表单不显示字段、分组或底部装饰分隔线；桌面端使用左侧名称说明、右侧控件统一对齐的单列行式布局，移动端改为名称在上、控件在下。只有页签正文滚动，保存/取消保持可见。页签切换直接复用内存中的配置副本，不新增持久化字段。
