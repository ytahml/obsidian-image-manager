...
图床配置还包含“远程对象管理”区域。旧配置默认关闭；开启后可设置管理前缀（空值为当前 Bucket 根）、每页对象数量（默认 100，通用范围 1–1000）和额外公共 URL 基础路径（一行一个）。保存会去除前缀首尾 `/`，但不推断上传模板或修改中间路径。G3 不开放预览模式或删除开关，二者固定为安全默认值。
