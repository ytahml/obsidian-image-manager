...
└── renderImageHosting    // 图床（带 heading）
    ...
    ├── 上传路径模板 (text)
    ├── 自定义引用模板 (text，上传后生效；{url} 必填，支持 {alt})
    ├── 上传后自动替换 (toggle)
    ├── 粘贴时自动上传 (toggle)
...
上传后自定义引用模板可用于 HTML 等文本格式。`{url}` 是必填变量，`{alt}` 使用上传文件名或现有引用的 alt 文本；空白或未包含 `{url}` 的模板不会生效，运行时安全回退到默认 Markdown 引用。当前仅覆盖上传后的远程引用，不改变本地粘贴、Markdown/Wiki 转换、重命名或整理流程。
