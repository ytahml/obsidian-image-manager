...
`HostingConfigModal` 使用固定基础信息和“图床配置 / 远程管理”两个页签。图床配置页以轻量分组连续承载服务商 endpoint/region/bucket/凭据，以及上传路径、公共 URL 或 Custom 请求映射；不为只有少量字段的上传设置单独页签。表单不为每个字段增加外框，只有页签正文滚动，保存/取消保持可见；桌面两列、移动单列。页签切换直接复用内存中的配置副本，不新增持久化字段。
