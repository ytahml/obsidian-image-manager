...
- 搜索框（300ms 防抖）
- 排序（名称、大小、修改时间、创建时间）
- 引用状态筛选下拉框：全部引用状态、已引用、孤立图片
- 打开本地视图后异步扫描引用：绿色“已引用”、橙色“孤立图片”，失败时灰色“无法判断”
- 只有当前孤立卡片可多选；选择跨搜索、排序、筛选和重绘保留
- 批量删除在确认前和执行时各重新扫描一次，使用 `fileManager.trashFile` 移入 Obsidian 配置的回收站
...
G3 后该 Modal 是本地/图床的共同外壳：默认本地视图保持搜索、引用状态筛选和预览，并通过 `local-orphan-management.ts` 共享 fresh 扫描、选择校验和顺序回收站删除。引用状态扫描完成前筛选下拉框禁用；扫描成功后可在完整本地结果中切换全部、已引用或孤立图片，选择跨筛选保留。切换页签或关闭后，迟到的本地扫描结果通过 generation 隔离。图床视图位于独立的 `remote-image-browser.ts`，只有用户点击扫描、继续扫描或刷新时才请求列表 Provider。
...
远程大图预览在标题下以与本地预览一致的“标签 + 值”信息行显示“远程路径”和完整对象 key。loading 放在图片画布区域内，成功后由同一区域直接显示图片，标题和图片本身即为成功反馈，不再显示冗余的“预览已加载”。失败反馈也显示在图片区域内，并保留人工重试入口。
...
本地引用状态筛选、安全回收站删除、远程路径信息以及默认展开的完整本地引用列表已于 2026-07-25 通过用户真实 Obsidian 验收。
...
- 全选/全不选
- 显示总大小
- 批量删除复用图片浏览器的 fresh 二次校验和 `fileManager.trashFile` 回收站边界
...
- “远程管理”页签通过生产 Provider 的 `list` capability 门控，当前对阿里云 OSS、S3-compatible 与七牛 Kodo 显示；Custom 在配置正文提示当前不支持远程管理
...
S3 页签门控和 URL alias 多行示例/无效行警告已于 2026-07-19 通过用户 Obsidian 验收；七牛 Kodo 的远程管理页签及扫描、公开/私有预览和删除已于 2026-07-25 通过用户真实 Obsidian 验收。阿里云 OSS 使用相同页签和公共门禁，等待真实 Obsidian 人工验收。
