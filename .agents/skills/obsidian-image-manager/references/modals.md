...
G3 后该 Modal 是本地/图床的共同外壳：默认本地视图保持搜索、孤立筛选和预览，并通过 `local-orphan-management.ts` 共享 fresh 扫描、选择校验和顺序回收站删除。切换页签或关闭后，迟到的本地扫描结果通过 generation 隔离。图床视图位于独立的 `remote-image-browser.ts`，只有用户点击扫描、继续扫描或刷新时才请求列表 Provider。
