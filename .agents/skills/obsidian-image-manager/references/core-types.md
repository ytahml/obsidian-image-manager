...
interface ImageManagerSettings {
    ...
    customReferenceTemplate: string;  // 上传后自定义引用模板；{fileUrl} 必填，支持 alt、文件元数据与固有宽高
...
- `RemoteFolderListRequest` / `RemoteFolderListPage`：Provider 无关的单层虚拟目录分页契约；返回完整规范化前缀并保留不透明 cursor。S3 与阿里云 OSS 只从 `CommonPrefixes` 生成目录，不把 `Contents` 猜测为文件夹。
