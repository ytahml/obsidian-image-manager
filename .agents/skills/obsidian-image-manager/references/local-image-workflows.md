...
选择只允许当前明确孤立的卡片，并跨搜索、排序、筛选与重绘保留。“选择当前结果”只加入当前搜索与引用筛选结果中的可选项；“清空选择”清除包括当前隐藏项在内的全部选择。普通复选框点击建立 Shift 锚点，Shift 点击另一复选框按当前完整筛选/排序顺序选择或取消两者间的可选项，区间外选择不变；搜索、筛选、排序或扫描结果变化会清除锚点。卡片点击仍只打开预览，区间计算不依赖 DOM。少量结果按内容高度贴顶，不拉伸卡片。
