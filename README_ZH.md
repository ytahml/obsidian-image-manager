...
远程对象管理目前仅支持 S3 兼容存储。“其他引用 URL 基础路径”应每行填写一个 HTTP(S) 基础路径，每个路径需截止到对象 key 开始前的位置；不要使用逗号或分号分隔。

每批最多选择 20 个符合资格的对象。删除前需要输入所选数量，并勾选已知晓云端删除无法撤销；请求最多 2 个并发，不会自动重试。成功结果统一显示“请求成功”，资源空间是否释放取决于服务商的删除与版本管理策略。建议使用专用 Bucket 或目录前缀、授予最小权限，并通过重新扫描 S3 范围验证结果。插件最多保留 200 条已完成删除请求的脱敏本地诊断记录；它们不参与远程存在、引用状态或删除资格判断，也不包含临时预览 URL 和凭据。
