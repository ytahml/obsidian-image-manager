...
- **Delegated 模式**将这部分本地管理权交给 Obsidian 或外部附件管理者。本插件只观察公开 Vault 事件和当前可解析的引用；不依赖管理者的 ID 或固定延迟，再把稳定附件接力给可选的自动图床上传。
- 每次 delegated 粘贴或拖放都必须唯一匹配“附件—本次新增引用”。上传重试、替换和本地清理前都会重新验证；状态歧义或已失效时，保留本地文件和引用。
...
flowchart LR
    ...
    K --> L{"是否保留本地副本？"}
    L -->|是| M["保留本地附件"]
    L -->|否| N["Fresh 扫描和变更保护期"]
    N -->|仍有引用或已变化| M
    N -->|没有本地引用| O["移入 Obsidian 回收站"]
```
...
在 **Delegated** 模式下，自动上传会等待“新附件—本次新增引用”能唯一匹配后再执行；它只替换该精确引用。上传期间事务变得歧义或发生相关变更时，会保留本地文件。
