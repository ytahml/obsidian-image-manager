...
Obsidian 图片管理插件 — 支持图片压缩、图床上传、引用格式转换、本地与远程图片浏览等功能。
...
兼容性说明：与 Attachment Management 0.12.1 配合使用 delegated 模式时，请关闭“保留本地副本”。该插件的附件重命名流程可能再次写回本地引用并覆盖远端引用；首版不支持该组合下的 `keepLocalCopy=true`。
