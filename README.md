...
> **注意**：本插件不适用使用 Wiki 图片格式（`![[image.png]]`）的用户。如果你希望从 Wiki 格式转换到标准 Markdown 格式（`![alt](image.png)`）进行图片管理，本插件非常合适。
...
- **引用格式** — 插入图片引用时使用标准 Markdown 格式（`![alt](image.png)`）
