...
- If **Keep Local Copy** is disabled, cleanup uses Obsidian's trash only after the attachment is no longer protected by recent lifecycle changes and fresh scans still show no local references. The scan conservatively recognizes Markdown/Wiki links (including table-escaped Wiki size syntax), HTML, frontmatter, Canvas, and Excalidraw resources; ambiguous references are kept and cannot be selected for cleanup. This protects the current vault; it cannot establish that an image is unused outside the vault.
