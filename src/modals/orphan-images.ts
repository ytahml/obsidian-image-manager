...
export class OrphanImagesModal extends Modal {
            ...
            contentEl.createDiv({
                ...
                text:
                    result.indeterminate.length > 0
                        ? t("modal.orphan.noOrphansUnknown", {
                              unknown: String(result.indeterminate.length),
                          })
                        : t("modal.orphan.noOrphans"),
            });
            return;
        }
    ...
    private renderList() {
        ...
        if (this.sizeEl) {
            const totalSize = this.orphans.reduce(
    ...
    private async confirmDelete() {
        ...
        try {
            ...
            console.error(
                "[ImageManager] Failed to revalidate orphan image selection:",
                error,
            );
        }
    }
    ...
    private async deleteSelected(paths: ReadonlySet<string>) {
        const result = await trashValidatedLocalOrphans(this.app, paths, () =>
            this.scanProtectedOrphans(),
        );
        new Notice(
            t("modal.orphan.deleted", {
