...
} from "../src/uploaders/note-images";
...
describe("note image upload references", () => {
    ...
    it("reads live editor content and resolves a fully encoded Unicode path", async () => {
        ...
        const getFirstLinkpathDest = vi.fn((path: string) =>
            path === "assets/中文 image.png" ? image : null,
        );
        const read = vi.fn();
        const app = {
            workspace: {
                getActiveViewOfType: vi.fn(() => ({
                    ...
                    editor: {
                        getValue: () =>
                            "![中文](assets/%E4%B8%AD%E6%96%87%20image.png)",
                    },
                })),
            },
    ...
    it("unwraps angle-bracket destinations before resolving them", () => {
        ...
        const app = {
            metadataCache: {
                getFirstLinkpathDest: vi.fn((path: string) =>
                    path === "assets/my image.png" ? image : null,
                ),
            },
            vault: {
                getAbstractFileByPath: vi.fn(),
        ...
        expect(
            resolveLocalImageReference(app, note, "<assets/my%20image.png>"),
        ).toEqual({
            status: "resolved",
            file: image,
        });
    ...
    it("excludes every remote scheme, protocol-relative URL, data URL and blob URL", async () => {
        ...
        expect(result.references.map((item) => item.reference.path)).toEqual([
            "local.png",
        ]);
    });
});
