...
- 不自动扫描整个 Bucket；空前缀也必须由用户手动发起并确认。
- 不默认加载所有缩略图或原图。
...
| 2026-07-15 | 默认 metadata-only、手动分页、手动预览 | 避免打开浏览器即产生大量远程流量 | G3、G4 |
...
| 2026-07-16 | 远程浏览器复用 Modal 外壳，远程视图独立；pageSize per-hosting 默认 100 | 控制现有本地浏览器改动范围，并保持不同图床的分页偏好 | G0、G3 |
